...
const ChatCard: React.FC<ChatCardProps> = ({ chat }) => {
      ...
      {chat.map((chatItem, index) => {
        ...
        return (
          ...
          >
            ...
            <Card
              className={`w-fit py-2 px-4 max-w-[80%] overflow-hidden ${isUser
  ...
  components={{
    ...
    p({ node, children, ...props }) {
      // 🛡️ Don't wrap <pre> or <code> inside <p>
      const isOnlyCodeBlock = node.children?.some(
        (child: any) => child.tagName === 'code' || child.tagName === 'pre'
      );
      if (isOnlyCodeBlock) return <>{children}</>;
