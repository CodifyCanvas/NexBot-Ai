...
import ProfileImageDialog from "./ProfileForm/profile-image-dialog";
    ...
    <div>
      <div className="flex flex-col sm:flex-row transition-all duration-300 items-center space-y-4">
        ...
        <div className="text-center transition-all duration-300 flex flex-col items-center sm:items-start gap-4 sm:pl-10 sm:-mt-4 mb-4 sm:mb-0">
          <p className="text-white/70 w-fit py-1 px-3 sm:py-2 text-sm sm:px-5 outline-1 outline-white/20 rounded-full">
            {user?.admin ? "Admin" : "Owner"}
          </p>
      ...
      {/* Render the dialog and control its open state */}
      {isDialogOpen && <ProfileImageDialog onClose={closeDialog} />}
    </div>
  );
};
